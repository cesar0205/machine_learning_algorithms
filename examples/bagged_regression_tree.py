from supervised_learning.trees.bagged_trees import BaggedRegressionTree
import numpy as np
...
def main():
    ...
    model = BaggedRegressionTree()
    model.fit(X_train, y_train)
    print("Test score: ", model.score(X_test, y_test))
    plt.plot(X, y)
    pred = model.predict(X.reshape(T, 1))
    plt.plot(X, pred, 'o')
    plt.title("Bagged regression tree. Fitting the sin(x) function")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.show()
